...
public class ActiveBuild
{
    ...
    public ReplayBuildComponent? ReplayBuildComponent { get; set; }
}

public sealed record BreakpointInfo(Breakpoint Value, string Label);
...
public class ReplayHelper
{
    ...
    {
        var durationSeconds = _replayDetails.Replay.Duration;

        return _replayDetails.Replay.Players
            .SelectMany(p => p.Spawns)
            .Select(s => s.Breakpoint)
            .Distinct()
            .OrderBy(b => b)
            .Select(b => new BreakpointInfo(b, ToLabel(b, durationSeconds)))
            .ToList();
    }

    private static string ToLabel(Breakpoint breakpoint, int durationSeconds)
    {
        return breakpoint switch
        {
            Breakpoint.None => string.Empty,
            Breakpoint.Min5 => "5:00",
            Breakpoint.Min10 => "10:00",
            Breakpoint.Min15 => "15:00",
            Breakpoint.All => $"{FormatDuration(durationSeconds)} (end)",
            _ => breakpoint.ToString()
        };
    }
    ...
    private static string FormatDuration(int totalSeconds)
    {
        ...
        // Display mm:ss (or hh:mm:ss if desired)
        return ts.Hours > 0
    ...
    public void UpdateBuilds()
    {
        ...
        {
            build.ReplayBuildComponent?.Update();
        }
    }

    public void ToggleBuild(ToonIdDto toonId)
    {
        var player = GetPlayer(toonId);
        if (player == null) return;

        if (_activeBuilds.Remove(toonId))
            return; // acts as toggle OFF

        _activeBuilds[toonId] = new() { Player = player }; // toggle ON
    }

    public bool PlayerBuildIsActive(ToonIdDto toonId) =>
        _activeBuilds.ContainsKey(toonId);
}