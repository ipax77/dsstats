...
public static class DsUnitTypeExtenstions
{
    public static IEnumerable<string> GetUnitTypes(this DsUnitType dsUnitType)
        => Enum.GetValues<DsUnitType>()
               .Cast<DsUnitType>()
               .Where(flag =>
                    flag != DsUnitType.None &&
                    dsUnitType.HasFlag(flag))
               .Select(f => f.ToString());
}