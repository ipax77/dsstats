...
public interface IBuildsService
{
    ...
    Task<List<BuildUpgradeTimingDto>> GetUpgradeTimings(BuildsRequest request, bool includeAnecdotal = false, CancellationToken token = default);
    Task<List<BuildGasTimingDto>> GetGasTimings(BuildsRequest request, bool includeAnecdotal = false, CancellationToken token = default);
