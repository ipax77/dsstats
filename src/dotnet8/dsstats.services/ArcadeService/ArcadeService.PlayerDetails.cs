...
public partial class ArcadeService
{
    ...
    public async Task<ArcadePlayerDetails> GetPlayerDetails(PlayerId playerId, CancellationToken token)
    {
    ...
    private async Task<int> GetArcadePalyerId(PlayerId playerId, CancellationToken token = default)
    {
        using var scope = scopeFactory.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<ReplayContext>();
    ...
    private async Task<List<KeyValuePair<GameMode, int>>> GetGameModes(int arcadePlayerId, ReplayContext context, CancellationToken token)
    {
        ...
        if (gamemodes.Any())
        {
            return gamemodes.Select(s => new KeyValuePair<GameMode, int>((GameMode)s.Key, s.Count)).ToList();
        }
        else
        {
            return new();
    ...
    public async Task<ArcadePlayerMoreDetails> GetMorePlayerDatails(PlayerId playerId, RatingType ratingType, CancellationToken token)
    {
        using var scope = scopeFactory.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<ReplayContext>();
    ...
    private async Task<double> GetTeamRating(PlayerId playerId, RatingType ratingType, bool inTeam, ReplayContext context, CancellationToken token)
    {
    ...
    private async Task<List<PlayerTeamResult>> GetPlayerTeammates(PlayerId playerId, RatingType ratingType, bool inTeam, ReplayContext context, CancellationToken token)
    {
    ...
    private async Task<int> GetMaxPos(RatingType ratingType)
    {
        ...
        if (!memeoryCache.TryGetValue(memKey, out int maxPos))
        {
            ...
            maxPos = await context.ArcadePlayerRatings
                .Where(x => x.RatingType == (int)ratingType)
