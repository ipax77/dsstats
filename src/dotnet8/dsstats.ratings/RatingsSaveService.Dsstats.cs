...
public partial class RatingsSaveService
{
    ...
    int replayRatingId = 0, int replayPlayerRatingId = 0)
    {
        ...
        for (int i = 0; i < ratings.Count; i++)
        {
            ...
            foreach (var rp in rating.RepPlayerRatings)
            {
                ...
                {
                    ReplayRatingInfoId = replayPlayerRatingId,
    ...
    public async Task SaveDsstatsPlayerRatings(Dictionary<int, Dictionary<PlayerId, CalcRating>> mmrIdRatings, bool isContinue)
    {
        ...
        {
            await WirteDsstatsPlayerRatingsCsvFile(mmrIdRatings, playerIds);
        ...
        await SetRatingChange(connectionString);
    }
                                                        ...
                                                        Dictionary<PlayerId, int> playerIdDic)
    {
        ...
        foreach (var ent in mmrIdRatings)
        {
            foreach (var entCalc in ent.Value.Values)
            {
                ...
                ratings.Add(GetDsstatsPlayerRatingCsvLine(entCalc, ent.Key, i, playerId));
