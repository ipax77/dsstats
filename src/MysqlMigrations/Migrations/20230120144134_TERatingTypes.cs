...
{
    public partial class TERatingTypes : Migration
    {
		...
		WHILE timePeriod_counter < timePeriod_max DO
			...
			CREATE TABLE IF NOT EXISTS TEMP_changetable AS (
				SELECT `p`.`PlayerId`, `pr`.`PlayerRatingId`, ROUND(COALESCE(SUM(`r1`.`RatingChange`), 0.0), 2) AS `RatingChange`
				  ...
				  WHERE ((`r`.`GameTime` > DATE_ADD(CURDATE(), INTERVAL CAST(days AS signed) day)) AND `p`.`UploaderId` IS NOT NULL) AND (`r0`.`RatingType` = ratingType_counter) AND (`pr`.`RatingType` = ratingType_counter)
...
END";
            ...
            var sp2 = @"
    DROP PROCEDURE IF EXISTS SetPlayerRatingPos;
    CREATE PROCEDURE `SetPlayerRatingPos`()
BEGIN
	SET @pos = 0;
