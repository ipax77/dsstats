...
[TestClass]
public class TimelineComponentLifecycleTests
{
    [TestMethod]
    public async Task LoadFailureReportsRequestAndReplacementCanRecover()
    {
        var first = new StatsRequest { Type = StatsType.Timeline };
        var second = new StatsRequest { Type = StatsType.Timeline, Interest = Commander.Nova, TimePeriod = TimePeriod.AllTime };
        var stats = new Mock<IStatsService>();
        stats.Setup(x => x.GetStatsAsync<TimelineResponse>(StatsType.Timeline, first, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new HttpRequestException("Offline"));
        stats.Setup(x => x.GetStatsAsync<TimelineResponse>(StatsType.Timeline, second, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new TimelineResponse());
...
.BuildServiceProvider();
        ...
        {
            [nameof(TimelineComponent.StatsRequest)] = request,
            [nameof(TimelineComponent.OnLoadFailed)] = EventCallback.Factory.Create<StatsRequest>(this, r => failed.TrySetResult(r)),
            [nameof(TimelineComponent.OnResponseLoaded)] = EventCallback.Factory.Create<(StatsRequest Request, TimelineResponse Response)>(this,
        ...
        Assert.AreEqual(TimePeriod.AllTime, second.TimePeriod);
    ...
    {
        var first = new StatsRequest { Type = StatsType.Timeline };
        var second = new StatsRequest { Type = StatsType.Timeline, Interest = Commander.Kerrigan };
        var firstLoad = new TaskCompletionSource<TimelineResponse>();
        var secondLoad = new TaskCompletionSource<TimelineResponse>();
...
.BuildServiceProvider();
        ...
        var callback = EventCallback.Factory.Create<(StatsRequest Request, TimelineResponse Response)>(this,
            result => { reported.Add(result.Request); completed.TrySetResult(); });
        ParameterView Parameters(StatsRequest request) => ParameterView.FromDictionary(new Dictionary<string, object?>
        {
            [nameof(TimelineComponent.StatsRequest)] = request,
            [nameof(TimelineComponent.OnResponseLoaded)] = callback
        });
        ...
        stats.Verify(x => x.GetStatsAsync<TimelineResponse>(StatsType.Timeline, second, It.IsAny<CancellationToken>()), Times.Once);
        Assert.IsNull(renderer.Error);
    }

    // Exercise the real load/parameter lifecycle without instantiating charts or making JS calls.
    public sealed class HeadlessTimeline : TimelineComponent
    {
...
#pragma warning disable BL0006 // A minimal renderer is intentionally used for lifecycle tests.
    private sealed class LifecycleRenderer : Renderer
    {
        ...
        public LifecycleRenderer(IServiceProvider services) : base(services, services.GetRequiredService<ILoggerFactory>())
            => componentId = AssignRootComponentId(InstantiateComponent(typeof(HeadlessTimeline)));
