...
[TestClass]
public class SynergyComponentLifecycleTests
{
    [TestMethod]
    public async Task LoadFailureReportsRequestAndReplacementCanRecover()
    {
        var first = new StatsRequest { Type = StatsType.Synergy };
        var second = new StatsRequest { Type = StatsType.Synergy, Interest = Commander.Nova, TimePeriod = TimePeriod.AllTime };
        var stats = new Mock<IStatsService>();
        stats.Setup(x => x.GetStatsAsync<SynergyResponse>(StatsType.Synergy, first, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new HttpRequestException("Offline"));
        stats.Setup(x => x.GetStatsAsync<SynergyResponse>(StatsType.Synergy, second, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new SynergyResponse());
...
.BuildServiceProvider();
        ...
        {
            [nameof(SynergyComponent.StatsRequest)] = request,
            [nameof(SynergyComponent.OnLoadFailed)] = EventCallback.Factory.Create<StatsRequest>(this, r => failed.TrySetResult(r)),
            [nameof(SynergyComponent.OnResponseLoaded)] = EventCallback.Factory.Create<(StatsRequest Request, SynergyResponse Response)>(this,
    ...
    {
        var first = new StatsRequest { Type = StatsType.Synergy };
        var second = new StatsRequest { Type = StatsType.Synergy, Interest = Commander.Kerrigan };
        var firstLoad = new TaskCompletionSource<SynergyResponse>();
        var secondLoad = new TaskCompletionSource<SynergyResponse>();
...
.BuildServiceProvider();
        ...
        var callback = EventCallback.Factory.Create<(StatsRequest Request, SynergyResponse Response)>(this,
            result => { reported.Add(result.Request); completed.TrySetResult(); });
        ParameterView Parameters(StatsRequest request) => ParameterView.FromDictionary(new Dictionary<string, object?>
        {
            [nameof(SynergyComponent.StatsRequest)] = request,
            [nameof(SynergyComponent.OnResponseLoaded)] = callback
        });
        ...
        stats.Verify(x => x.GetStatsAsync<SynergyResponse>(StatsType.Synergy, second, It.IsAny<CancellationToken>()), Times.Once);
        Assert.IsNull(renderer.Error);
    }

    // Exercise the real load/parameter lifecycle without instantiating charts or making JS calls.
    public sealed class HeadlessSynergy : SynergyComponent
    {
...
#pragma warning disable BL0006 // A minimal renderer is intentionally used for lifecycle tests.
    private sealed class LifecycleRenderer : Renderer
    {
        ...
        public LifecycleRenderer(IServiceProvider services) : base(services, services.GetRequiredService<ILoggerFactory>())
            => componentId = AssignRootComponentId(InstantiateComponent(typeof(HeadlessSynergy)));
