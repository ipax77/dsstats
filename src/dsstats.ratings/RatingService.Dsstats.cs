...
public partial class RatingService
{
    private async Task ProduceDsstatsRatings(bool recalc)
    {
        ...
        await ratingSaveService.SaveDsstatsPlayerRatings(ratingRequest.MmrIdRatings, ratingRequest.SoftBannedPlayers, ratingRequest.Continue);
    }
    ...
    private async Task<CalcRatingRequest?> GetDsstatsCalcRatingRequest(ReplayContext context)
    {
        ...
        var oldestReplayTime = todoReplays.OrderBy(o => o.GameTime)
            .Select(s => s.GameTime)
            .First();

        if (oldestReplayTime < latestRatingsProduced)
        {
            return ratingRequest;
        }
