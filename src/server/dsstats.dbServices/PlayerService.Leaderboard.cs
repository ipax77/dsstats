...
public partial class PlayerService
{
    ...
    public async Task<List<PlayerRatingListItem>> GetRatings(PlayerRatingsRequest request, CancellationToken token = default)
    {
        try
        {
            long skip = ((long)request.Page - 1) * request.PageSize + request.Skip;
            if (skip < 0 || skip > int.MaxValue || request.Take <= 0)
                return [];

            bool cacheTop = request.PageSize < 10
                && request.Page == 1 && request.Skip == 0
                && string.IsNullOrEmpty(request.Name) && request.RegionId == 0
                && !request.IsActive && (request.Orders == null || request.Orders.Count == 0);
            var cacheKey = $"topRatings_{request.RatingType}_{request.Take}";
            if (cacheTop && memoryCache.TryGetValue(cacheKey, out List<PlayerRatingListItem>? cached) && cached is not null)
                return cached;

            await using var context = await contextFactory.CreateDbContextAsync(token);
            var query = GetOrderedRatings(GetRatingsQueryable(request, context), request);
            var rawData = await query.AsNoTracking().Skip((int)skip).Take(request.Take).ToListAsync(token);
            var items = rawData.Select(s => s.ToPlayerRatingListItem()).ToList();
            if (cacheTop)
                memoryCache.Set(cacheKey, items, TimeSpan.FromHours(1));
            return items;
        }
        catch (OperationCanceledException) { }
        catch (Exception ex)
        ...
        PlayerRatingsRequest request)
    {
        if (request.Orders == null || request.Orders.Count == 0)
        {
            ...
            return query.OrderByDescending(x => x.Rating).ThenBy(x => x.PlayerId);
        }
        ...
        foreach (var order in request.Orders)
        {
            switch (order.Column)
            {
                ...
                case nameof(PlayerRatingListItemRaw.MainCount):
                    orderedQuery = orderedQuery == null
                        ? (order.Ascending ? query.OrderBy(x => (x.Games == 0 ? 0.0 : (double)x.MainCount / x.Games)) : query.OrderByDescending(x => (x.Games == 0 ? 0.0 : (double)x.MainCount / x.Games)))
                        : (order.Ascending ? orderedQuery.ThenBy(x => (x.Games == 0 ? 0.0 : (double)x.MainCount / x.Games)) : orderedQuery.ThenByDescending(x => (x.Games == 0 ? 0.0 : (double)x.MainCount / x.Games)));
                    break;
                ...
                case nameof(PlayerRatingListItemRaw.Wins):
                    orderedQuery = orderedQuery == null
                        ? (order.Ascending ? query.OrderBy(x => (x.Games == 0 ? 0.0 : (double)x.Wins / x.Games)) : query.OrderByDescending(x => (x.Games == 0 ? 0.0 : (double)x.Wins / x.Games)))
                        : (order.Ascending ? orderedQuery.ThenBy(x => (x.Games == 0 ? 0.0 : (double)x.Wins / x.Games)) : orderedQuery.ThenByDescending(x => (x.Games == 0 ? 0.0 : (double)x.Wins / x.Games)));
                    break;

                case nameof(PlayerRatingListItemRaw.Mvps):
                    orderedQuery = orderedQuery == null
                        ? (order.Ascending ? query.OrderBy(x => (x.Games == 0 ? 0.0 : (double)x.Mvps / x.Games)) : query.OrderByDescending(x => (x.Games == 0 ? 0.0 : (double)x.Mvps / x.Games)))
                        : (order.Ascending ? orderedQuery.ThenBy(x => (x.Games == 0 ? 0.0 : (double)x.Mvps / x.Games)) : orderedQuery.ThenByDescending(x => (x.Games == 0 ? 0.0 : (double)x.Mvps / x.Games)));
                    break;
        ...
        return (orderedQuery ?? query.OrderByDescending(x => x.Rating)).ThenBy(x => x.PlayerId);
    }


    private static IQueryable<PlayerRatingListItemRaw> GetRatingsQueryable(PlayerRatingsRequest request, DsstatsContext context)
    {
