...
public sealed class StatsPrompt
{
    public required int FormatVersion { get; init; }
    public required string PromptVersion { get; init; }
    public required string[] SystemMessages { get; init; }
    public required StatsPromptExample[] Examples { get; init; }
    ...
    public static StatsPrompt Load(string path)
    {
        ...
        var fields = typeof(StatsQueryPlan).GetProperties().Select(p => JsonNamingPolicy.CamelCase.ConvertName(p.Name)).ToHashSet();
        if (!fields.SetEquals(properties.EnumerateObject().Select(p => p.Name)) ||
            ...
            throw new InvalidDataException("The response schema fields must match the stats query contract.");
