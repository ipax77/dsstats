...
public partial class ReplaysService : IReplaysService
{
    ...
    private async Task AddPlayerInfos(ReplaysRequest request, ReplaysResponse response, CancellationToken token)
    {
        ...
        if (request.MauiInfo)
        {
            await AddDsstatsPlayerInfos(request, response, token);
            return;
        }
    ...
    private async Task AddDsstatsPlayerInfos(ReplaysRequest request, ReplaysResponse response, CancellationToken token)
    {
        if (request.PlayerId is null)
        {
            return;
